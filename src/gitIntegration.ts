...
export function registerGitIntegration(context: vscode.ExtensionContext) {
  ...
  context.subscriptions.push(
    vscode.workspace.onDidChangeConfiguration(async e => {
      if (e.affectsConfiguration('worklogGenerator.includeWorklogInCommitMessage') && 
        ...
        if (lastGeneratedWorklog && inputBox) {
          ...
          // Reset the setting
          await vscode.workspace.getConfiguration('worklogGenerator').update(
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('worklog-ai.generateWorklogBeforeCommit', async () => {
      const autoGenerate = vscode.workspace.getConfiguration('worklogGenerator').get('autoGenerateOnCommit', false);
      ...
      if (autoGenerate) {
        try {
          ...
          const llmProvider = vscode.workspace.getConfiguration('worklogGenerator').get('defaultLlmProvider', 'gemini');
          const worklogStyle = vscode.workspace.getConfiguration('worklogGenerator').get('defaultWorklogStyle', 'dsu');
